...
export default function Results({ state, onRestart, darkMode, onOpenMobileSidebar }: ResultsProps) {
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto">
        {!reviewing ? (
          <>
            <Card className={`mb-6 border-2 ${darkMode ? "bg-slate-800 border-slate-600" : "border-blue-200"}`}>
              <CardHeader
                className={`text-center ${darkMode ? "bg-slate-900" : "bg-gradient-to-r from-blue-50 to-indigo-50"}`}
              >
                <CardTitle className={`text-4xl ${darkMode ? "text-white" : "text-gray-900"}`}>Exam Results</CardTitle>
              </CardHeader>
              <CardContent className="pt-8 pb-8">
                <div className="grid grid-cols-3 gap-4 mb-8">
                  <div
                    className={`text-center p-4 rounded-lg border ${
                      darkMode ? "bg-green-900 border-green-700 text-green-100" : "bg-green-50 border-green-200"
                    }`}
                  >
                    <div className={`text-3xl font-bold ${darkMode ? "text-green-300" : "text-green-600"}`}>
                      {correct}
                    </div>
                    <div className={`text-sm mt-1 ${darkMode ? "text-green-200" : "text-green-800"}`}>Correct</div>
                  </div>
                  <div
                    className={`text-center p-4 rounded-lg border ${
                      darkMode ? "bg-red-900 border-red-700 text-red-100" : "bg-red-50 border-red-200"
                    }`}
                  >
                    <div className={`text-3xl font-bold ${darkMode ? "text-red-300" : "text-red-600"}`}>
                      {incorrect}
                    </div>
                    <div className={`text-sm mt-1 ${darkMode ? "text-red-200" : "text-red-800"}`}>Incorrect</div>
                  </div>
                  <div
                    className={`text-center p-4 rounded-lg border ${
                      darkMode ? "bg-blue-900 border-blue-700 text-blue-100" : "bg-blue-50 border-blue-200"
                    }`}
                  >
                    <div className={`text-3xl font-bold ${darkMode ? "text-blue-300" : "text-blue-600"}`}>
                      {percentage}%
                    </div>
                    <div className={`text-sm mt-1 ${darkMode ? "text-blue-200" : "text-blue-800"}`}>Score</div>
                  </div>
                </div>

                <div className={`w-full rounded-full h-4 mb-4 ${darkMode ? "bg-slate-700" : "bg-gray-200"}`}>
                  <div
                ...
                <p className={`text-center font-semibold mb-6 ${darkMode ? "text-slate-100" : "text-gray-700"}`}>
                  You answered {correct} out of {state.questions.length} questions correctly
                </p>
                ...
                <div className="space-y-3">
                  ...
                  {onOpenMobileSidebar && (
                    <Button
                      ...
                      className={`w-full ${darkMode ? "bg-slate-700 border-slate-600 text-slate-100 hover:bg-slate-600" : "bg-transparent"}`}
                    >
                      Change Subject
                    </Button>
                  )}
                  <Button onClick={onRestart} className="w-full bg-green-600 hover:bg-green-700 text-white">
                    Return to Home
                  </Button>
                </div>
              </CardContent>
          ...
          <>
            <div className="flex justify-between items-center mb-6 flex-wrap gap-2">
              <h1 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>Review Answers</h1>
            ...
            <Card className={darkMode ? "bg-slate-800 border-slate-700 mb-6" : "mb-6"}>
              <CardContent className="pt-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm font-semibold ${darkMode ? "text-slate-400" : "text-gray-600"}`}>
                ...
                <div className="space-y-3">
                  {current.opts.map((option, index) => {
                    ...
                    let bgColor = darkMode ? "bg-slate-700 border-slate-600" : "bg-white border-gray-300"
                    if (isCorrectOption) {
                    ...
                    return (
                      <div key={index} className={`w-full p-4 text-left border-2 rounded-lg ${bgColor}`}>
                        <div className="flex items-center justify-between">
                          <span className={darkMode ? "text-slate-100" : "text-gray-900"}>{option}</span>
                          {isCorrectOption && (
                            <span
                              className={darkMode ? "text-green-300 font-semibold" : "text-green-600 font-semibold"}
                            >
                              Correct
                            </span>
                          )}
                          {isUserAnswer && !isCorrect && (
                            <span className={darkMode ? "text-red-300 font-semibold" : "text-red-600 font-semibold"}>
                              Your answer
                            </span>
                          )}
                        </div>
                ...
                {userAnswer !== undefined && !isCorrect && (
                  ...
                  >
                    ...
                    <p className={`text-sm mt-1 ${darkMode ? "text-yellow-100" : "text-yellow-900"}`}>
                      <strong>Correct answer:</strong> {current.opts[current.answer]}
                    </p>
                  </div>
                )}
            ...
            <Button onClick={() => setReviewing(false)} className="w-full bg-green-600 hover:bg-green-700">
              Back to Results
            </Button>
          </>
        )}
