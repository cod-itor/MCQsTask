...
export default function ReadingPractice({ onBack, darkMode, onOpenMobileSidebar }: ReadingPracticeProps) {
  ...
  const renderQuestion = (q: ReadingQuestion, index: number) => {
    ...
    let inputClass = `mx-1 md:mx-2 px-2 md:px-3 py-1.5 border rounded-md inline-flex items-center justify-between text-sm min-w-[120px] md:min-w-[140px] focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
      darkMode ? "bg-slate-800 border-slate-600 text-white" : "bg-white border-gray-300 hover:bg-gray-50"
    }`

    let textInputClass = `mx-1 md:mx-2 px-2 md:px-3 py-1.5 border rounded-md inline-block text-sm min-w-[120px] md:min-w-[140px] focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
                                  ...
                                  >
                                    <Check
                                      className={`mr-2 h-4 w-4 ${
                                        userAnswer === opt ? "opacity-100" : "opacity-0"
                                      }`}
                                    />
                                    {opt}
                                  </CommandItem>
  ...
  const passagePanel = (
    <div className={`w-full h-full p-4 md:p-8 overflow-y-auto ${!isMobile ? (darkMode ? "border-r border-slate-700 bg-slate-900" : "border-r border-gray-200 bg-white") : ""}`}>
  ...
  const questionsPanel = (
    <div className={`w-full h-full relative flex flex-col overflow-hidden ${darkMode ? "bg-slate-800" : "bg-[#f8fcf8]"}`}>
      {/* Global Options Sticky Box */}
      {currentPassage.globalOptions && currentPassage.globalOptions.length > 0 && (
        <div className={`flex-shrink-0 p-4 md:p-6 border-b ${darkMode ? "bg-slate-800/95 border-slate-700" : "bg-[#f8fcf8]/95 border-emerald-100"} backdrop-blur-sm z-10 shadow-sm`}>
          <div className={`border-2 rounded-xl p-4 md:p-5 ${darkMode ? "border-emerald-700/50 bg-emerald-950/20" : "border-emerald-600/30 bg-white"}`}>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-3 md:gap-x-6 md:gap-y-4">
              {currentPassage.globalOptions.map((opt, i) => (
                <div 
                  key={i}
                  draggable
                  onDragStart={(e) => {
                  ...
                  className={`cursor-grab active:cursor-grabbing px-2.5 py-1 md:px-3 md:py-1.5 rounded-md text-xs md:text-sm font-medium transition-all ${
                    darkMode 
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        <Card className={`border-0 shadow-none bg-transparent ${darkMode ? "text-slate-200" : ""}`}>
          <CardContent className="p-0">
            <div className={`mb-6 md:mb-8 p-4 md:p-5 rounded-xl ${darkMode ? "bg-blue-900/20 text-blue-200 border border-blue-900/50" : "bg-blue-50 text-blue-900 border border-blue-100"}`}>
              <h3 className="font-semibold text-base md:text-lg mb-1 md:mb-2">Questions 1 - {currentPassage.questions.length}</h3>
              <p className="opacity-80 text-sm md:text-base">Complete the sentences below. Type your answer, select from the dropdown, or drag & drop an option.</p>
            </div>
            ...
            <div className="mt-8 md:mt-12 flex flex-col gap-4 md:gap-6">
              {isSubmitted && (
                <div className={`p-4 md:p-6 rounded-xl text-center font-bold text-xl md:text-2xl ${
                  darkMode ? "bg-slate-700 text-white" : "bg-white border-2 border-emerald-100 shadow-md text-emerald-900"
                }`}>
  ...
  return (
    <div className={`flex flex-col h-screen ${darkMode ? "bg-slate-900 text-white" : "bg-gray-50 text-gray-900"}`}>
      {/* Header */}
      <header
      ...
      <div className="flex flex-col flex-1 overflow-hidden relative">
        {isMobile ? (
          <Tabs defaultValue="passage" className="flex flex-col flex-1 overflow-hidden w-full h-full">
            <div className={`px-4 pt-3 pb-2 flex-shrink-0 border-b z-10 ${darkMode ? 'border-slate-700 bg-slate-900/95' : 'border-gray-200 bg-white/95'} backdrop-blur-sm`}>
              <TabsList className="grid w-full grid-cols-2 h-11">
                <TabsTrigger value="passage" className="text-sm">Reading Passage</TabsTrigger>
                <TabsTrigger value="questions" className="text-sm">Questions</TabsTrigger>
              </TabsList>
            </div>
            <TabsContent value="passage" className="flex-1 overflow-hidden mt-0 data-[state=active]:flex data-[state=active]:flex-col border-0 p-0 m-0 focus-visible:ring-0">
              {passagePanel}
            </TabsContent>
            <TabsContent value="questions" className="flex-1 overflow-hidden mt-0 data-[state=active]:flex data-[state=active]:flex-col border-0 p-0 m-0 focus-visible:ring-0">
              {questionsPanel}
            </TabsContent>
          </Tabs>
        ) : (
          <div className="flex flex-row flex-1 w-full h-full overflow-hidden">
            <div className="flex-1 w-1/2 overflow-hidden">{passagePanel}</div>
            <div className="flex-1 w-1/2 overflow-hidden">{questionsPanel}</div>
          </div>
        )}
      </div>
    </div>
