...
function PageContent() {
  ...
  const [page, setPage] = useState<
    "home" | "input" | "practice" | "exam-setup" | "exam" | "results" | "mcqs" | "about"
  >("home")
  ...
  const handleStartExam = (duration: number, numQuestions: number) => {
    ...
    const selectedMcqs = allMcqs.sort(() => Math.random() - 0.5).slice(0, numQuestions)
  ...
  const showSidebar = ["practice", "exam-setup", "exam", "results", "mcqs"].includes(page)
  ...
  return (
    <>
      <Navbar
        ...
        currentPage={page === "home" ? "home" : page === "about" ? "about" : page === "mcqs" ? "mcqs" : "home"}
      />
      ...
      <main className={`min-h-screen ${darkMode ? "dark" : ""}`}>
        <div className={`min-h-screen transition-colors duration-300 ${darkMode ? "bg-gray-950" : "bg-gray-50"}`}>
          <div className="flex" style={{ height: "calc(100vh - 80px)", marginTop: "80px" }}>
            {/* Desktop Sidebar */}
            <div className={`hidden md:block w-64 border-r ${!showSidebar ? "md:hidden" : ""}`}>
            ...
            <div className="flex-1 overflow-auto">
              ...
              {page === "input" && <MCQInput onMcqsLoaded={() => setPage("mcqs")} darkMode={darkMode} />}
              {page === "practice" && (
                <PracticeTest
                  onBack={() => setPage("mcqs")}
                  darkMode={darkMode}
